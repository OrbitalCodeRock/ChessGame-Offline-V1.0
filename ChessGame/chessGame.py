...
class Piece(object): 
    def __init__(self, color, name, position, img, isSelected = False, isChecking = False, pinningPiece = None, possibleMoves = [], isDefending = []):
        self.color = color
        self.name = name
...
class Pawn(Piece):
    def __init__(self, color, name, position, img, isSelected = False, isChecking = False, pinningPiece = None, possibleMoves = [], isDefending = [], doubleMoveLastTurn = False):
...
class Rook(Piece):
    def __init__(self, color, name, position, img, isSelected = False, isChecking = False, pinningPiece = None, possibleMoves = [], isDefending = [], hasMoved = False):
        super().__init__(color, name, position, img, isSelected, isChecking, pinningPiece, possibleMoves, isDefending)
...
def main():
    ...
    moveDot = pygame.image.load('Black Dot.png')
        ...
        if(piece.name == 'pawn'):
            ...
            for x in enemyList:
                    ...
                    if(x.position[1] == piece.position[1] and (x.position[0] == piece.position[0] - 84 or x.position[0] == piece.position[0] + 84)):
                       if(x.name == 'pawn' and x.doubleMoveLastTurn == True):
            ...
            if(piece.position[1] > 0 and piece.position[1] < 588 and not blockingSingleMove):
            ...
            for x in friendlyList:
                    if(x.position[0] == piece.position[0] and x != piece):
                            if(x.position[1] < piece.position[1]):
                                    if(closestPieceUp == None or closestPieceUp.position[1] < x.position[1]):
                                            closestPieceUp = x
                                            piece.isDefending.append(x.position)
                            elif(x.position[1] > piece.position[1]):
                                    if(closestPieceDown == None or closestPieceDown.position[1] > x.position[1]):
                                            closestPieceDown = x
                                            piece.isDefending.append(x.position)
                    if(x.position[1] == piece.position[1] and x != piece):
                            if(x.position[0] < piece.position[0]):
                                    if(closestPieceLeft == None or closestPieceLeft.position[0] < x.position[0]):
                                            closestPieceLeft = x
                                            piece.isDefending.append(x.position)
                            elif(x.position[0] > piece.position[0]):
                                    if(closestPieceRight == None or closestPieceRight.position[0] > x.position[0]):
                                            closestPieceRight = x
                                            piece.isDefending.append(x.position)
            for x in enemyList:
                    if(x.position[0] == piece.position[0]):
        ...
        if(piece.name == 'king'):
           ...
           if(piece.hasMoved == False):
               ...
               if(piece.color == 'white'):
                   ...
                   rook1 = whiteRook1
                   rook2 = whiteRook2
               if(rook1.hasMoved == False and piece.color == rook1.color):
                   castleBlocked = False
                   for x in friendlyList:
                   ...
                   for x in enemyList:
                        ...
                        if(breakLoop == True):
                           break
                   if(castleBlocked == False):
                        piece.possibleMoves.append([piece.position[0] - 168, yPosition])
               if(rook2.hasMoved == False and piece.color == rook2.color):
                   castleBlocked = False
                   for x in friendlyList:
                ...
                if(piece.pinningPiece != None):
                    for x in piece.possibleMoves:
                        if(piece.pinningPiece.name == "rook"):
                            ...
                            if(canMove == False and x != piece.pinningPiece.position and removeList.count(x) < 1):
                                removeList.append(x)
                ...
                if(loneChecker.name == 'pawn'):
                    ...
                    for x in piece.possibleMoves:
                        if(loneChecker.position == x):
                            ...
                            if((x[1] != loneChecker.position[1] or x[0] >= friendlyKing.position[0]) and removeList.count(x) < 1):
                                removeList.append(x)
                        elif(loneChecker.position[0] > friendlyKing.position[0]):
                            if((x[1] != loneChecker.position[1] or x[0] <= friendlyKing.position[0]) and removeList.count(x) < 1):
                                removeList.append(x)
                        elif(loneChecker.position[1] < friendlyKing.position[1]):
                            if((x[0] != loneChecker.position[0] or x[1] >= friendlyKing.position[1]) and removeList.count(x) < 1):
                                removeList.append(x)
                        elif(loneChecker.position[1] > friendlyKing.position[1]):
                            if((x[0] != loneChecker.position[0] or x[1] <= friendlyKing.position[1]) and removeList.count(x) < 1):
                                removeList.append(x)
                    ...
                    for x in piece.possibleMoves:
                        ...
                        if(canMove == False and x != loneChecker.position and removeList.count(x) < 1):
                            removeList.append(x)
                    ...
                    if(checkingLikeRook == False):
                        ...
                        for x in piece.possibleMoves:
                            if(loneChecker.position == x):
                                continue
                            elif(loneChecker.position[0] < friendlyKing.position[0] and loneChecker.position[1] == friendlyKing.position[1] and removeList.count(x) < 1):
                                if(x[1] != loneChecker.position[1] or x[0] >= friendlyKing.position[0]):
                                    removeList.append(x)
                            elif(loneChecker.position[0] > friendlyKing.position[0] and loneChecker.position[1] == friendlyKing.position[1] and removeList.count(x) < 1):
                                if(x[1] != loneChecker.position[1] or x[0] <= friendlyKing.position[0]):
                                    removeList.append(x)
                            elif(loneChecker.position[1] < friendlyKing.position[1] and loneChecker.position[0] == friendlyKing.position[0] and removeList.count(x) < 1):
                                if(x[0] != loneChecker.position[0] or x[1] >= friendlyKing.position[1]):
                                    removeList.append(x)
                            elif(loneChecker.position[1] > friendlyKing.position[1] and loneChecker.position[0] == friendlyKing.position[0] and removeList.count(x) < 1):
                                if(x[0] != loneChecker.position[0] or x[1] <= friendlyKing.position[1]):
                                    removeList.append(x)
    ...
    def checkForEndgame():
        ...
        if(whiteNoMoves == True and turn == 'black'):
            for piece in blackList:
                if piece.isChecking == True:
                    whiteInCheck = True
        ...
        if(blackNoMoves == True and turn == 'white'):
            for piece in whiteList:
                if piece.isChecking == True:
                    blackInCheck = True
                            ...
                            if(x[0] < mousePos[0] < x[0] + 84 and x[1] < mousePos[1] < x[1] + 84):
                                for y in enemyList:
                                    if(y.position == x):
                                        ...
                                        enemyList.remove(y)
                                        break
                                    elif(piece.name == 'pawn' and y.name == 'pawn' and y.doubleMoveLastTurn == True and (piece.position[0] - 84 == y.position[0] or piece.position[0] + 84) and piece.position[1] == y.position[1]):
                                ...
                                if(askForReset == True):
                                    while True:
                                        response = input("Would you like to reset the board? 'Yes' or 'no'\n").casefold()
